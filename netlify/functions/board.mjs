...
export default async (req) => {
  ...
  try {
    ...
    if (method === 'POST' && action === 'create') {
      ...
      if (items.length !== 25) {
        return json(400, { error: 'A board needs exactly 25 items.' });
      }
      for (const item of items) {
        if (!item || typeof item.text !== 'string' || !item.text.trim()) {
          return json(400, { error: 'Every square needs text.' });
        }
      }
      ...
      if (!boardId) {
        return json(500, { error: 'Could not generate a board code, try again.' });
      }
    ...
    if (!id) {
      return json(400, { error: 'Missing board id.' });
    }

    // Fetch a board
    if (method === 'GET') {
      const record = await store.get(id, { type: 'json' });
      if (!record) return json(404, { error: 'No board found with that code.' });
      return json(200, record);
    }
    ...
    if (method === 'POST' && action === 'toggle') {
      ...
      if (!itemExists) return json(400, { error: 'That square does not exist on this board.' });
    ...
    console.error(err);
    // process.env.NETLIFY_DEV is 'true' under the local CLI dev server —
    // safe to return the real error there; keep production responses generic.
